...
/* Single-shot motion variants. MotionConfig reducedMotion="user" flattens these
   automatically for visitors who ask their OS for reduced motion. */
const reveal = {
...
/** The one primary action, reused in the hero and the closing CTA. Hoisted to
    module scope so toggling `busy` updates it in place instead of remounting. */
function GoogleButton({ busy, onClick, label, className }: { busy: boolean; onClick: () => void; label: string; className?: string }) {
  return (
    <Button
      type="button"
      variant="outline"
...
function Eyebrow({ children }: { children: ReactNode }) {
  return <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brand">{children}</p>
}
              ...
              <motion.div {...reveal} className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="flex flex-col items-start justify-center rounded-2xl border border-dashed border-border p-5">
                  ...
                  <p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">
                    {t('A new space appears and your AI can use it at once.', '新的區塊一出現，你的 AI 就能立刻用上。')}
                  </p>
                </div>
              </motion.div>
            </div>
        ...
        <footer className="border-t border-border/60">
          <div className="mx-auto flex max-w-6xl flex-col gap-5 px-5 py-10 sm:flex-row sm:items-center sm:justify-between sm:px-8">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[12px] text-muted-foreground">{t('FSRS · MCP · Knowledge graph', 'FSRS · MCP · 知識圖譜')}</span>
